<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" label-width="80px">
      <dc-search :isMoreShow.sync="isMoreShow">
        <el-form-item label="文件名" prop="fileName">
          <el-input v-model="queryParams.fileName" placeholder="请输入文件名" clearable size="small" @keyup.enter.native="handleQuery" />
        </el-form-item>
        ...
        <el-form-item label="文件后缀" prop="fileSuffix">
          <el-input v-model="queryParams.fileSuffix" placeholder="请输入文件后缀" clearable size="small" @keyup.enter.native="handleQuery" />
        </el-form-item>
        ...
        <el-form-item label="上传人" prop="createBy">
          <el-input v-model="queryParams.createBy" placeholder="请输入上传人" clearable size="small" @keyup.enter.native="handleQuery" />
        </el-form-item>
        <el-form-item label="服务商" prop="service">
          <el-input v-model="queryParams.service" placeholder="请输入服务商" clearable size="small" @keyup.enter.native="handleQuery" />
        </el-form-item>
        <template slot="opt">
    ...
    <el-row :gutter="8" class="dc-opt-row">
      <el-col :span="1.5">
        <el-button type="primary" plain size="small" @click="handleFile" v-hasPermi="['system:oss:upload']">上传文件</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="primary" plain size="small" @click="handleImage" v-hasPermi="['system:oss:upload']">上传图片</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="primary" plain size="small" :disabled="multiple" @click="handleDelete" v-hasPermi="['system:oss:remove']">批量删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button :type="previewListResource ? 'primary' : 'primary'" plain size="small" @click="handlePreviewListResource(!previewListResource)" v-hasPermi="['system:oss:edit']"
          >预览开关 : {{ previewListResource ? '禁用' : '启用' }}</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button type="primary" plain size="small" @click="handleOssConfig" v-hasPermi="['system:oss:list']">配置管理</el-button>
      </el-col>
    </el-row>
    ...
    <el-table
      ...
      <el-table-column label="对象存储主键" prop="ossId" v-if="false" min-width="120" show-overflow-tooltip />
      <el-table-column label="文件名" prop="fileName" min-width="120" show-overflow-tooltip />
      <el-table-column label="原名" prop="originalName" min-width="120" show-overflow-tooltip />
      <el-table-column label="文件后缀" prop="fileSuffix" min-width="120" show-overflow-tooltip />
      <el-table-column label="文件展示" prop="url" show-overflow-tooltip>
      ...
      <el-table-column label="上传人" prop="createBy" min-width="120" show-overflow-tooltip />
      <el-table-column label="服务商" prop="service" sortable="custom" min-width="120" show-overflow-tooltip />
      <el-table-column label="操作" class-name="small-padding" fixed="right" width="120" v-if="checkPermi(['system:oss:download', 'system:oss:remove'])">
        <template slot-scope="scope">
          <el-button size="mini" type="text" @click="handleDownload(scope.row)" v-hasPermi="['system:oss:download']">下载</el-button>
