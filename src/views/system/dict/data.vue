<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" label-width="80px" size="small">
      <dc-search :isMoreShow.sync="isMoreShow">
        <el-form-item label="字典名称：" prop="dictType">
          <el-select v-model="queryParams.dictType" filterable @change="handleQuery">
            <el-option v-for="item in typeOptions" :key="item.dictId" :label="item.dictName" :value="item.dictType" />
          </el-select>
        </el-form-item>
        <el-form-item label="字典标签：" prop="dictLabel">
          <el-input v-model="queryParams.dictLabel" placeholder="请输入" clearable @keyup.enter.native="handleQuery" />
        </el-form-item>
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange" ref="table" :height="autoHeight">
      ...
      <el-table-column label="字典标签" prop="dictLabel" min-width="120" show-overflow-tooltip>
        <template slot-scope="scope">
          ...
          <dc-tag v-else :type="scope.row.listClass == 'primary' ? '' : scope.row.listClass">{{ scope.row.dictLabel }}</dc-tag>
        </template>
      </el-table-column>
      ...
      <!-- <el-table-column label="操作"  class-name="small-padding" fixed="right" width="120" v-if="checkPermi(['system:dict:edit','system:dict:remove'])">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" size="small">
        ...
        <el-form-item label="数据标签" prop="dictLabel">
          <el-input v-model="form.dictLabel" placeholder="请输入(最多20个字符)" maxlength="20" clearable />
        </el-form-item>
        <el-form-item label="数据键值" prop="dictValue">
          <el-input v-model="form.dictValue" placeholder="请输入(最多20个字符)" maxlength="20" clearable />
        </el-form-item>
        <el-form-item label="样式属性" prop="cssClass">
          <el-input v-model="form.cssClass" placeholder="请输入(最多20个字符)" maxlength="20" clearable />
        </el-form-item>
        <el-form-item label="显示排序" prop="dictSort">
          <el-input-number v-model="form.dictSort" controls-position="right" :min="0" :max="2147483647" placeholder="请输入(0-2147483647)" />
        </el-form-item>
        <el-form-item label="回显样式" prop="listClass">
          <el-select v-model="form.listClass" filterable>
